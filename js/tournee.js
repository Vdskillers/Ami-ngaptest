...
/* ════════════════════════════════════════════════════════════════════════
   ⚡ Code Planning hebdomadaire DÉPLACÉ vers planning.js (2026-04-23)
   ────────────────────────────────────────────────────────────────────────
   Les fonctions suivantes sont désormais dans planning.js et restent
   accessibles globalement (déclarations top-level → window) :
      _planningKey, _savePlanning, _loadPlanning, _clearPlanning,
      _syncPlanningStorage, _syncPlanningToServer, _syncPlanningFromServer,
      _restorePlanningIfNeeded, _renderPlanningIfVisible, refreshPlanning,
      _getCabinetMode, _setCabinetMode, planningWeekNav,
      planningToggleCabinetView, _planningInitCabinetUI, planningGenerateCabinet,
      generatePlanningFromImport, renderPlanning, planningOptimiseCabinetWeek,
      _planningDeleteCotation, _planningRemovePatient, _planningResetAll,
      window._planningReassignIDE, window._planningToggleIDE,
      window._planningCabinetMode (alias), window._planningInitCabinetUI

   Les listeners ('app:nav', 'ui:navigate' pour pla, 'app:update' pour
   importedData, 'ami:login' pour restauration) sont aussi dans planning.js.

   ⚠️ tournee.js → storeImportedData() appelle _syncPlanningStorage() :
   fonctionne car planning.js définit cette fonction au top-level (= globale).
   Charger planning.js APRÈS tournee.js dans index.html.
════════════════════════════════════════════════════════════════════════ */
...
async function autoFacturation(patient){
  ...
  try{
    ...
    // ⚡ invoice_number existant : si le patient a déjà une cotation antérieure
    // (re-cotation, correction), on transmet le N° au worker pour faire un
    // PATCH (mise à jour) au lieu d'un INSERT (création) → évite le doublon
    // dans l'Historique des soins.
    const _invExist = patient._cotation?.invoice_number || null;
    // ⚡ Date du soin : priorité date stockée (cohérence si autoFacturation
    // rappelée tardivement en batch) > date du jour.
    const _dateSoin = patient._cotation?._tournee_date
                   || (typeof _localDateStr === 'function' ? _localDateStr() : new Date().toISOString().split('T')[0]);
    // ⚡ Heure RÉELLE du soin. Priorité :
    ...
    //      plus tard en batch.
    //   2. patient._cotation._heure_reelle : tagué par _validateCotationLive.
    //   3. new Date() : fallback direct (auto-facturation déclenchée sans
    //      clic "Terminer" préalable).
    // Jamais patient.heure_soin / patient.heure_preferee (contrainte planifiée).
    const _heureSoin = patient?._done_at
    ...
    const d = await apiCall('/webhook/ami-calcul', {
      ...
      // ⚡ Champs INDISPENSABLES pour l'Historique des soins ──────────
      // Tout patient coté en Mode Uber Médical doit remonter avec :
      //  ─ patient_nom   : affiché dans la colonne Patient (évite avatar "?")
      //  ─ patient_id    : rattachement IDB / Supabase (lien fiche carnet)
      //  ─ invoice_number: PATCH d'une correction (sinon doublon)
      //  ─ date_soin     : colonne Date — figée à la cotation, pas glissante
      //  ─ heure_soin    : colonne Heure — heure RÉELLE du clic "Terminer"
      date_soin:  _dateSoin,
      heure_soin: _heureSoin,
      _live_auto: true,
      ...(_patNom    ? { patient_nom:    _patNom    } : {}),
      ...(_patId     ? { patient_id:     _patId     } : {}),
      ...(_invExist  ? { invoice_number: _invExist  } : {}),
